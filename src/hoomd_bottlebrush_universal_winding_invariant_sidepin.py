...
helix_arg_group.add_argument(
    '--side_pinning_frequency_kb', 
    type=float, 
    default=0, 
    help="The frequency of particles pinned to one side of a chromosome, kb. "
         "Pins are chosen at random, but adjusted to loop mids. Redundant pins (i.e. pins on the same loop) are prunned.")
helix_arg_group.add_argument(
    '--side_pinning_strength', 
    type=float, 
    default=3, 
    help="The strength of particles pinned to one side of a chromosome, kb.")
...
if args.conf_type == 'cylindrical':
    ...
    p.SIDE_PINNING_FREQUENCY_KB = args.side_pinning_frequency_kb
    p.SIDE_PINNING_FREQUENCY = args.side_pinning_frequency_kb * 1000 / p.BP_PP
    p.SIDE_PINNING_STRENGTH = args.side_pinning_strength
...
def make_sim_name(p):
    ...
    if p.CONFINEMENT_TYPE == 'cylindrical':
        ...
        sim_name_dict.update(dict(
            ...
            WallE=p.CYL_CONF_EPSILON,
            SPinFreq=p.SIDE_PINNING_FREQUENCY_KB,
            SPinStr=p.SIDE_PINNING_STRENGTH,
        ))
...
if p.SIDE_PINNING_FREQUENCY:
    loop_type = 'nested_loop' if p.OUTER_LOOP_SIZE else 'root_loop'
    all_loops_to_pin = snapshot.bonds.group[snapshot.bonds.typeid == snapshot.bonds.types.index(loop_type)]
    loop_lens = all_loops_to_pin[:,1] - all_loops_to_pin[:,0]
    n_pins = int(np.ceil(np.sum(loop_lens) / p.SIDE_PINNING_FREQUENCY))
    loops_to_pin = np.random.choice(len(all_loops_to_pin), n_pins, replace=True, p=loop_lens/np.sum(loop_lens))
    loops_to_pin = np.unique(loops_to_pin)
    loopmids_to_pin = np.mean(all_loops_to_pin[loops_to_pin], axis=1).astype(int)

    forces['side_pinning'] = shared_hoomd_poly.force_pin_loopmids_to_side(
        particle_ids_to_pin=loopmids_to_pin,
        strength=0,
        direction=(1,0,0),
    )
    p.SIDE_PINNING_PARTICLE_IDS = loopmids_to_pin
...
def enable_side_pinning(sim, forces, strength=p.SIDE_PINNING_STRENGTH, direction=(1,0,0)):
    if 'side_pinning' in forces:
        if sim.timestep // p.FIRE_BLOCK_SIZE == 20:
            forces['side_pinning'].constant_force['monomer'] = np.array(direction) * strength
            logging.debug(f"Turned side pinning on at timestep {sim.timestep}")


main_run_updaters = []
if p.CONFINEMENT_TYPE == 'cylindrical_helix':
    main_run_updaters.append(update_angular_pinning)

if p.SIDE_PINNING_FREQUENCY:
    main_run_updaters.append(enable_side_pinning)
